...
-- Asegurar columna permitido en permisos
ALTER TABLE permisos ADD COLUMN IF NOT EXISTS permitido boolean DEFAULT false;
...
DROP POLICY IF EXISTS authenticated_all_permisos ON permisos;
CREATE POLICY authenticated_all_permisos ON permisos
  FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');

-- Seed: Catálogos del sistema
INSERT INTO catalogos (tipo, valor, color, orden, activo) VALUES
  -- Prioridades (usadas en quejas, SACP)
  ...
  ('estado_queja', 'Abierta',     'amber',  2, true),
  ('estado_queja', 'En Proceso',  'orange', 3, true),
  ('estado_queja', 'Cerrada',     'green',  4, true),
...
-- Seed: Permisos base por rol
INSERT INTO permisos (rol, modulo, accion, permitido) VALUES
  ('admin',       'quejas',       'ver',     true),
  ('admin',       'quejas',       'crear',   true),
  ('admin',       'quejas',       'editar',  true),
  ('admin',       'quejas',       'eliminar',true),
  ('admin',       'configuracion','ver',     true),
  ('admin',       'configuracion','editar',  true),
  ('coordinador', 'quejas',       'ver',     true),
  ('coordinador', 'quejas',       'crear',   true),
  ('coordinador', 'quejas',       'editar',  true),
  ('coordinador', 'quejas',       'eliminar',false),
  ('revisor',     'quejas',       'ver',     true),
  ('revisor',     'quejas',       'crear',   false),
  ('revisor',     'quejas',       'editar',  true),
  ('revisor',     'quejas',       'eliminar',false),
  ('usuario',     'quejas',       'ver',     true),
  ('usuario',     'quejas',       'crear',   true),
  ('usuario',     'quejas',       'editar',  false),
  ('usuario',     'quejas',       'eliminar',false);

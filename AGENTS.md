...
**Stack:** Next.js 16.2.12 (App Router, React 19.2.4) + Supabase (Postgres + Auth + Realtime) + TanStack Query v5 + Zustand v5 + Tailwind CSS v4 + lucide-react + sonner + chart.js/react-chartjs-2 + recharts.
...
app/
  ...
  q/[token]/ page.tsx    formulario público de quejas (sin auth, categorías fijas obligatorias + adjuntos Drive)
components/
  ...
  ui/  índice barrel (components/ui/index.ts): Button, Input, Label, Textarea, Select, Badge, Table(Head/HeaderCell/Row/Cell), Skeleton, Switch, StatusDot, FieldGroup, EmptyState, PageHeader, DropdownMenu(+Content/Item/Separator), Tooltip, Avatar, ScrollArea, Separator
  header/  NotificationsDropdown.tsx, UserMenu.tsx (refactor de UI en componentes — Header.tsx aún tiene versión inline equivalente activa)
  ...
  configuracion/ai/   RoutingTable, ProviderList, ProviderFormModal, ModelTestModal, CacheConfig, constants, actions — REFACTOR PARALELO DORMANTE, NO importado por ninguna página (cablear desde AIProvidersManager si se adopta)
...
lib/
  ...
  ai/         types.ts (AIProvider, AIRuta, AIRouting, ArchivoIA), aiFactory.ts (crearClienteIA multi-proveedor + blindaje Gemini + sanitización + resolverGeminiFlashAuto)
              modelDiscovery.ts (obtenerModelosDisponibles + OpenRouter free-only + DB cache), modelMemory.ts (último éxito/fallos/latencia + penalización condicional), modelTesting.ts (testearModelo/testearProveedor + persistencia BD)
  store/      auth-store (Zustand: user + permisos + vistaActiva + init/login/logout/setVistaActiva), sidebar-store (collapsed), theme-store (HUÉRFANO, sin toggle)
  config/     navigation.ts (NAV_SECTIONS con icons lazy + PREFETCH_MAP; Sidebar aún usa su versión hardcodeada equivalente)
  constants/  index.ts, roles.ts (ROLE_VARIANTS/LABELS 6 roles), variants.ts (prioridadVariant, estadoVariant, estadoSACPVariant, estadoDocumentoVariant)
  utils/      index.ts, format.ts (formatBytes/Number/Date/DateTime), drive.ts (esDrive, getDriveFileUrl/Preview/Thumbnail)
hooks/
...
public/
  sounds/     8 MP3 autohospedados (notification-{info,success,popup,error} + game-{coin,void,hit,miss}) — originales de react-sounds
...
| `/q/[token]` | formulario público sin auth (valida token en `formularios_publicos` activo; crea vía RPC `crear_queja_publica`; sube adjuntos a Drive; muestra folio). Categoría = select FIJO obligatorio: Queja, Denuncia, Sugerencia, Reclamo, Felicitación (sin catálogo) | — |
...
20. **Notificaciones reales:** las mutaciones de quejas insertan notificaciones **dentro de los RPC** (`transicionar_queja`, `agregar_comentario_queja`, `crear_queja_publica`, `procesar_alertas_quejas`) dirigidas al responsable + admin/calidad activos. `notificacionService` lista/actualiza del Header. Campana conectada a `useNotificaciones` (no leídas, dropdown, marcar leída/todas, archivar, vaciar, refetch 60s, beep al subir).
...
26. **Realtime** (`useRealtimeSubscription`): `/quejas` suscribe a `quejas` (invalida quejas+estadísticas+dashboard); `Header`/`NotificationsDropdown` suscribe a `notificaciones` por `usuario_id` (badge + beep inmediato). Debounce de 750ms antes de invalidar.
...
35. **Testeo de modelos (modelTesting.ts):** `testearProveedor` ejecuta hasta 20 modelos con 2 prompts (corto + largo), guarda en BD (`ai_test_resultado_<id>`). UI con modal personalizado, progreso en tiempo real, Iniciar/Cancelar/Cerrar. Al sincronizar, excluye modelos con `ok:false` previo.
...
- **Sin escritores:** `informes_config`, `tareas`, `solicitudes_documentales`. `logs` sí lo escriben los RPC de quejas; `mail_queue` lo escribe `procesar_alertas_quejas` y `crear_queja_publica` (sin worker).
- **Capa muerta:** `lib/services/*Service.ts` (queja, auditoria, documento, proceso, reunion, riesgo, sacp, user, notificacion) no es importada por ninguna página; el flujo de quejas usa `quejaWorkflowService`. `reabrir_queja` (RPC 007) existe pero la UI usa `transicionar_queja` con `p_motivo_reapertura`. `theme-store` sin consumidores; `theme-toggle.tsx` no existe.
- **Refactor IA dormante:** `components/configuracion/ai/*` (RoutingTable, ProviderList, ProviderFormModal, ModelTestModal, CacheConfig, constants, actions) NO se importa desde ninguna página — es un refactor paralelo sin cablear. El componente activo es `components/configuracion/AIProvidersManager.tsx`. Igualmente `components/header/NotificationsDropdown.tsx`/`UserMenu.tsx` existen y NO se usan (Header.tsx mantiene versión inline), y `lib/config/navigation.ts` existe pero Sidebar usa su versión hardcodeada equivalente.
...
Gestor de IA dinámico, sin modelos fijos. Configuración en `configuraciones_sistema` (jsonb, RLS staff SELECT / admin write). La UI activa es `components/configuracion/AIProvidersManager.tsx` (tab IA de `/configuracion`). Existe `components/configuracion/ai/*` (refactor dormante, no conectado).
...
- **`testearModelo`**, **`testearProveedor`** (hasta 20 modelos, 2 prompts corto+largo, `onProgress` + `signal`, guarda en BD), **`obtenerResultadoTest`**, **`limpiarResultadoTest`**, **`modelosExcluidosPorTest`**.
